...
from .models import (GD2, CulturaPlantacao2, MediaGD2, Previsao2, SomaTermica2,
                     Temperatura2)
...
def calcular_gd(sender, instance, **kwargs):
    ...
    total_registros = Temperatura2.objects.filter(
    ...
    if total_registros % 10 == 0:
        ...
        ultimas_temperaturas = Temperatura2.objects.filter(
            fk_sensor=instance.fk_sensor
        ).order_by('-data')[:10]

        # Verifica a cultura associada ao sensor
        culturas = CulturaPlantacao2.objects.filter(
            fk_sensor=instance.fk_sensor)
        ...
        for cultura in culturas:
            ...
            GD2.objects.create(valor_gd=gd,
                               fk_cultura=cultura,
                               temperatura_max=temperatura_maior,
                               temperatura_min=temperatura_menor)


post_save.connect(calcular_gd, sender=Temperatura2)
...
def calcular_soma_termica(sender, instance, **kwargs):
    ...
    if SomaTermica2.objects.filter(fk_cultura=instance.fk_cultura).exists():  # noqa E501
        # Recupera o último valor de soma térmica
        soma_atual = SomaTermica2.objects.filter(
...
post_save.connect(calcular_soma_termica, sender=GD2)
...
def calcular_media_gd(sender, instance, **kwargs):
    ...
    MediaGD2.objects.create(
        valor_media=media,
        fk_cultura=instance.fk_cultura,
        data_insercao=formatted_datetime,)


post_save.connect(calcular_media_gd, sender=GD2)
...
def calcular_previsao(sender, instance, **kwargs):
    ...
    cultura = CulturaPlantacao2.objects.get(pk=instance.fk_cultura.pk)
    ...
    data_primeiro_gd = GD2.objects.filter(
    ...
    Previsao2.objects.create(valor_previsao=valor_previsao,
                             fk_cultura=cultura,
                             data_previsao=data_previsao,
                             data_insercao=formatted_datetime,)


post_save.connect(calcular_previsao, sender=MediaGD2)
